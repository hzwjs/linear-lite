...
- **发布门禁（补充）**：允许在同一分支内设置两个可发布 Gate，避免高优先级安全修复被重构阻塞：
  - **Gate A（必须先达成）**：P0 + P1（SQL 注入、密码哈希、越权、配置脱敏、事务）全部完成并验证通过，可优先合入/发布。
  - **Gate B**：P2 + 结构债（序号表、N+1、流式下载、`TaskService` 拆分）在 Gate A 后继续交付。
...
- **迁移门禁（新增）**：
  - 历史库上线前必须完成 backfill：`next_number = max(existing_task_number)+1`（按项目）。
  - backfill 未完成不得开启新版本写流量（避免 `task_key` 冲突）。
  - 提供可审计 SQL 清单：初始化脚本、校验脚本、回滚策略说明（保留表或回退步骤）。
