...
<template>
  <div class="breakdown-card">
    <h3 class="card-title">{{ t('analytics.statusBreakdown') }}</h3>
    <div v-if="!breakdown.length" class="card-empty">{{ t('analytics.noData') }}</div>
    <div v-else class="breakdown-list">
      <div v-for="item in breakdown" :key="item.status" class="breakdown-row">
        <span class="breakdown-name">{{ t(`status.${item.status}`, item.status) }}</span>
